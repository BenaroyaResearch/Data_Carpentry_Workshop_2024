...
We are investigating clinical data from an investigation lead by
Hamid involving covid sevirity and patient outcomes. The dataset 
is stored as a comma separated value (CSV) file. Each row holds 
information for a single animal, and the columns (not inclusive)
represent:

| Column          | Description                                  |
|-----------------|----------------------------------------------|
| Sample.ID       | Unique id for the observation                |
| drawDate        | identifier of draw date                      |
| Covid.ID        | alternative identifier of covid sample       |
| Score           | severity Score of infection                  |
| sex             | sex of the patient                           |
| age             | age of the patient                           |
| race            | ethinicity of the patient                    |
| bmi             | body mass indicator of the patient           |
| patientType     | Covid Status of the patient                  |
| HighestCare     | Highest level of Care required by patient    |
| Ever.On.Ventil  | was the patient ever intubated               |

There are additionally columns for flow markers for cell fraction
information.
...
file from github, names it "inputData.csv" and adds it to a
preexisting folder named "data_raw".

```{r, eval=FALSE, purl=TRUE}
download.file(url = "https://github.com/BenaroyaResearch/Data_Carpentry_Workshop_2022/blob/main/WORKSHOP_DATA/Bolouri_2021_subset.csv", 
destfile = "data_raw/inputData.csv")
```
...
surveys <- read_csv("data_raw/Bolouri_2021_subset.csv")
```
...
dataset, `read_csv()` reads `bmi` as `col_double` (a numeric data
type), and `race` as `col_character`. You have the option to specify
...
## * class: data frame
## * how many rows: 299,  how many columns: 29
...
## 3.
surveys_middle <- surveys[round(n_rows/2), ]
## 4.
...
integers. The columns `sex`, `HighestCare`, `patientType`, `race`, ...
...
1.  Change the columns `HighestCare` and 'sex'
    into a factor.

2.  Reorder HighestCare in the following order:
        - Neg, Floor, CCU, Outpatient
:::

::: solution
```{r, answer=TRUE, purl=FALSE}
surveys$sex <- factor(surveys$sex, levels = c("male","female"))
levels(surveys$sex)
surveys$HighestCare <- factor(surveys$HighestCare, levels = c("Neg", "Floor", "CCU", "Outpatient"))
levels(surveys$HighestCare)
summary(surveys$sex)
summary(surveys$HighestCare)
```
:::
:::
...
For instance, in one part of your analysis the draw date might need to be
encoded as factors (e.g., comparing average weights across draw dates) but in
another part of your analysis they may need to be stored as numeric
values (e.g., doing math operations on the drawdate). This conversion from
...
```{r, purl=TRUE}
## bar plot of the number of female and male patients assessed:
plot(surveys$sex)
```

However, as we saw when we used `summary(surveys$HighestCare)`, there are about
109 individuals for which the care information hasn't been recorded. To
...
```{r, results=TRUE, purl=FALSE}
care <- surveys$HighestCare
levels(care)
care <- addNA(care)
levels(care)
head(care)
levels(care)[5] <- "undetermined"
levels(care)
head(care)
```
...
::: solution
```{r, answer=TRUE, purl=FALSE}
levels(care)[3] <- "Negative"
care <- factor(care, levels = c("undetermined", "Negative", "CCU", "Floor", "Outpatient"))
plot(care)
```
:::
:::
