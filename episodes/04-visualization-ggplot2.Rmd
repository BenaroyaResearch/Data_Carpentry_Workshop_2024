...
```{r load-data, purl=FALSE, include=TRUE}
setwd("~/Desktop/codingBootcamp/R-data-carpentry-content/episodes/") #Note that this should be your working directory!

surveys_complete <- readr::read_csv("~/Desktop/codingBootcamp/R-data-carpentry-content/episodes/data/surveys_complete.csv")
...
```{r, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = age, y = bmi))
```
...
ggplot(data = surveys_complete, aes(x = age, y = bmi)) +
  geom_point()
```
...
Or to color each species in the plot differently, you could use a vector as an input to the argument **color**. **`ggplot2`** will provide a different color corresponding to different values in the vector. Here is an example where we color with **`sex`**:
...
Use what you just learned to create a scatter plot of `bmi` over
`sex` with the highest care showing in different colors.
...
ggplot(data = surveys_complete,
       mapping = aes(x = sex, y = bmi)) +
   geom_point(aes(color = HighestCare))
```
...
##
##  Use what you just learned to create a scatter plot of `bmi`
## over `sex` with the highest care showing in different colors.
## Is this a good way to show this type of data?
```

## Boxplot

We can use boxplots to visualize the distribution of bmi within each sex:

```{r boxplot, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = sex, y = bmi)) +
    geom_boxplot()
```
...
- Represent bmi on the log~10~ scale; see `scale_y_log10()`.
...
So far, we've looked at the distribution of bmi within sexes. Try making
a new plot to explore the distribution of another variable within each sex

- Create boxplot for `age`.  Overlay the boxplot layer on a jitter
  layer to show actual measurements.

:::::::: solution

```{r, answer=TRUE, purl=FALSE}
ggplot(data = surveys_complete, mapping = aes(x = sex, y = age)) +
geom_jitter(alpha = 0.3, color = "tomato") +
geom_boxplot(outlier.shape = NA)
```
...
- Add color to the data points on your boxplot according to the plot from which
  the sample was taken (`HighestCare`).
...
##  2. Represent bmi on the log10 scale; see `scale_y_log10()`.

##  3. Create boxplot for `age` overlaid on a jitter layer.

##  4. Add color to the data points on your boxplot according to the
##  plot from which the sample was taken (`HighestCare`).

```

## Plotting time series data

Let's calculate number of counts per age for each sex. First we need
to group the data and count records within each group:

```{r, purl=FALSE}
sex_counts <- surveys_complete %>%
  count(age, sex)
```

Timelapse data can be visualized as a line plot with age on the x-axis and
counts on the y-axis:

```{r first-time-series, purl=FALSE}
ggplot(data = sex_counts, aes(x = age, y = n)) +
     geom_line()
```
...
the aesthetic function to include `group = sex`:

```{r time-series-by-species, purl=FALSE}
ggplot(data = sex_counts, aes(x = age, y = n, group = sex)) +
    geom_line()
```

We will be able to distinguish sex in the plot if we add colors (using
`color` also automatically groups the data):

```{r time-series-with-colors, purl=FALSE}
ggplot(data = sex_counts, aes(x = age, y = n, color = sex)) +
    geom_line()
```
...
sex_counts %>%
    ggplot(mapping = aes(x = age, y = n, color = sex)) +
    geom_line()
```
...
sex_counts_graph <- surveys_complete %>%
    count(age, sex) %>%
    ggplot(mapping = aes(x = age, y = n, color = sex)) +
    geom_line()
...
So far we've been using line graphs to show these data, but as we can see, the age and the bmis of the patients do not well correlate with each other and do not show obvious linear trends. Let's represent them as a histogram instead to make a less confusing graph. Note, ebcause histograms are designed to measure the frequency or counts, you do not need to assign the y to the counts.
...
use it to make a age histogram for each sex:

```{r first-facet, purl=FALSE}
ggplot(data = sex_counts, aes(x = age)) +
    geom_histogram() +
    facet_wrap(facets = vars(sex))
```

Now we would like to split the line in each plot by the race of each individual
measured. To do that we need to make counts in the data frame grouped by `age`,
`race`, and `sex`:

```{r, purl=FALSE}
 age_sex_counts <- surveys_complete %>%
                      count(age, race, sex)
```

We can now make the faceted plot by splitting further by race using `color`
(within a single plot):

```{r facet-by-genus-and-sex, purl=FALSE}
ggplot(data = age_sex_counts, mapping = aes(x = age, color = race)) +
  geom_histogram() +
  facet_wrap(facets =  vars(sex))
```

We can also facet both by sex and race:

```{r average-weight-time-facet-both, purl=FALSE, fig.width=9.5}
ggplot(data = age_sex_counts,
       mapping = aes(x = age, color = sex)) +
  geom_histogram() +
...
Use what you just learned to create a plot that depicts how the average bmi
of each sex changes based on age. Hint: We're back to line graphs on this one!

:::::::: solution

```{r average-weight-time-series, answer=TRUE, purl=FALSE}
age_bmi <- surveys_complete %>%
                group_by(age, sex) %>%
                 summarize(avg_weight = mean(bmi))
ggplot(data = age_bmi, mapping = aes(x=age, y=avg_weight)) +
   geom_line() +
   facet_wrap(vars(sex)) +
   theme_bw()
```
...
##  average bmi of each sex changes based on age.
...
Now, let's change names of axes to something more informative than 'age'' and add a title to the figure:

```{r number-species-year-with-right-labels, purl=FALSE}
ggplot(data = age_sex_counts, aes(x = age, color = sex)) +
    geom_histogram() +
    facet_wrap(vars(race)) +
    labs(title = "Bmi versus age",
         x = "Age of Patient",
...
ggplot(surveys_complete, aes(x = sex, y = bmi)) +
    geom_boxplot() +
    grey_theme
```
...
plot_weight <- ggplot(data = surveys_complete, aes(x = sex, y = bmi)) +
  geom_boxplot() +
  labs(x = "Sex", y = "bmi") 

plot_count <- ggplot(data = age_sex_counts, aes(x = age, color = sex)) +
  geom_histogram() +
...
my_plot <- ggplot(data = age_sex_counts,
                  aes(x = age, color = sex)) +
    geom_histogram() +
    facet_wrap(vars(race)) +
    labs(title = "Observed bmi across age",
        x = "Patient Age",
        y = "Number of individuals") +
    theme_bw() +
...
ggsave("figure1.png", my_plot, width = 15, height = 10)
